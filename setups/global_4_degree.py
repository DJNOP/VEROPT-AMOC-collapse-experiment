...
class GlobalFourDegreeSetup(VerosSetup):
    ...
    def set_parameter(self, state):
        ...
        settings.description = "Global 4° with TKE/EKE; Veropt-wired params"
        ...
        # IDEMIX switches (off by default)
        ...
        if rp:
            ...
            # NOTE: new experiment: vary kappaH_min (Kh_min), not kappaM_min
            if "kappaH_min" in rp:
            ...
            logger.info(
                f"[veropt] runtime params -> "
                f"c_k={getattr(settings,'c_k',None)}, "
                f"kappaH_min={getattr(settings,'kappaH_min',None)}, "
                f"eke_c_k={getattr(settings,'eke_c_k',None)}, "
                f"eke_c_eps={getattr(settings,'eke_c_eps',None)}"
            )
    ...
    def set_diagnostics(self, state):
        ...
        state.diagnostics["overturning"].output_frequency = out_every
        state.diagnostics["overturning"].sampling_frequency = settings.dt_tracer

        state.diagnostics["energy"].output_frequency = out_every
        state.diagnostics["energy"].sampling_frequency = 86400.0
